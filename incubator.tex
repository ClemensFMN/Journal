...
Based on \cite{tucker2012appliedcombinatoris}.  At its heart, a generating function is a power series in one or more variables. By seleting the variables and some constraint on them, the product of the series can represent various combinatorial problems.
...
In \cite{tucker2012appliedcombinatoris}, they use a lot of machinery to get a closed-form expression for the generating function; here I'm lazy and use wxMaxima
...
\subsection{Crossing a rectangle}

Suppose we have a rectangle which we want to cross; ie go from point $A$ to point $C$. We can go upwards a length of $x$ (until point $B$), and then we move diagonally until we reach the destination $C$. The setup is shown in the following Figure.

\begin{figure}[H]
  \centering
  \includegraphics[scale=0.75]{images/crossing_rectangle.png}
\end{figure}

The total length of travel as function of $x$, $d(x)$, is given according to

\bee
d(x) = x + \sqrt{(H - x)^2 + W^2}, \quad 0 \leq x \leq H
\eee

The following Figure shows the value of $d(x)$ for $W = H = 1$.

\begin{figure}[H]
  \centering
  \includegraphics[scale=0.75]{images/crossing_rectangle_1.png}
\end{figure}

From the plot we can see the shortest path $d_0$ is achieved when $x = 0$; i.e. we use the diagonal for the whole trip. In this case $d(x) = d_0$ and we have

\bee
d_0 = \sqrt{H^2 + W^2}
\eee

The longest path $d_H$ is achieved when we go up and then left; this equals $x=H$ and we have

\bee
d_H = H + W
\eee

The same applies for other values of $W$ and $H$ (in particular also for $W \neq H$). In the case of a very wide rectangle, $W \gg H$, the expression for $d(x)$ can be approximated by

\bee
d(x) \approx x + W
\eee

We can define $r$ as the ratio between these two extremes,

\bee
r(\alpha) = \frac{d_H}{d_0} = \frac{H + W}{\sqrt{H^2 + W^2}} = \frac{1 + W / H}{\sqrt{1 + (W/H)^2}} = \frac{1 + \alpha}{\sqrt{1 + \alpha^2}}
\eee

where we have set $\alpha = W/H$. We can show (by setting $dr / d \alpha = 0$ and solving for $\alpha$) that the maximum of $r(\alpha)$ is reached for $\alpha=1$.


\subsection{Feynam's Trick for Integration}

Based on https://zackyzz.github.io/feynman.html

\paragraph{Classical example.} We consider the integral,

\bee
I = \int_0^1 \frac{x - 1}{\ln x} dx
\eee

This is tricky / impossible to do; however, Feynam's trick comes to the rescue. As a first step, we parametrize the integral (that's the tricky part to parametrize in such a way that the integral becomes solvable),

\bee
I(t) = \int_0^1 \frac{x^t - 1}{\ln x} dx
\eee

Next we differentiatie wrt to $t$ to get

\bee
I'(t) = \int_0^1 \frac{\partial }{\partial t}\frac{x^t - 1}{\ln x} dx = \int_0^1 x^t dx = \cdots = \frac{1}{t+1}
\eee

which can be solved very easily. However, we have obtained $I'(t)$ and we need to integrate this to get $I(t)$. In the general case, we have

\bee
\int_a^b f'(x) dx = f(b) - f(a)
\eee

In our example, $I = I(1)$ and also $I(0) = \int_0^1 \frac{x^0 - 1}{\ln x} dx = \int_0^1 0 dx = 0$ and therefore

\bee
I = I(1) = \int_0^1 I'(t) dt = \int_0^1 \frac{1}{t+1}dt = \left. \ln(1+t)\right|_0^1 = \ln(2) \qed
\eee
...
\bee
\boxed{
I = \int_0^1 \frac{x - 1}{\ln x} dx = \ln(2)
}
\eee

\paragraph{Another Example.} Let's now consider
...
\bee
I(t) = \int_{0}^\infty \frac{e^{-t(1 + x^2)}}{1 + x^2} dx
\eee

This works as

\begin{align*}
I(1) - I(\infty) &= \int_{0}^\infty \frac{e^{-(1 + x^2)}}{1 + x^2} dx - \int_{- \infty}^\infty \frac{e^{- \infty (1 + x^2)}}{1 + x^2} dx  \\
&= \int_{0}^\infty \frac{1}{1 + x^2} \left( e^{-(1 + x^2)} - e^{- \infty (1 + x^2)} \right) dx \\
&= \int_{0}^\infty \frac{1}{1 + x^2} \left( e^{-(1 + x^2)} - 0 \right) dx \\
&= \frac{1}{e} \int_{- \infty}^\infty \frac{e^{-x^2}}{1 + x^2} dx
\end{align*}

And our desired integral can be expressed as

\bee
\int_{0}^\infty \frac{e^{-x^2}}{1 + x^2} dx = e \left( I(1) - I(\infty) \right)
\eee

So, let's differentiate wrt $t$,

\begin{align*}
  I'(t) &= \int_{0}^\infty \frac{\partial }{\partial t}\frac{e^{-t(1 + x^2)}}{1 + x^2} dx = - \int_{- \infty}^\infty e^{-t(1 + x^2)} dx \\
  &= - e^{-t} \int_{0}^\infty e^{-t x^2} dx \\
  &= - e^{-t} \int_{0}^\infty e^{-t x^2} dx = - \frac{\sqrt{\pi} e^{-t}}{2 \sqrt{t}}
\end{align*}

Where we have solved the last integral by means of Maxima (nothing fancy, just a scaled normal distribution). From this we get

\begin{align*}
I &= e \left( I(1) - I(\infty) \right) = e \int_\infty^1 I'(t) dt \\
&= e \frac{\sqrt{\pi}}{2} \int_1^\infty \frac{e^{-t}}{\sqrt{t}} dt
\end{align*}

According to Maxima, the integral is

\bee
\int_1^\infty \frac{e^{-t}}{\sqrt{t}} dt = \Gamma(1/2, 1)
\eee

where $\Gamma$ is the \emph{incomplete Gamma function}. So we finally arrive at

\bee
\boxed{
I = \int_{0}^\infty \frac{e^{-x^2}}{1 + x^2} dx = e \frac{\sqrt{\pi}}{2} \Gamma(1/2, 1) \approx 0.672 \qed}
\eee

We can check the numerical values in Maxima as follows,

\begin{verbatim}
  (%i1)	quad_qag(%e^(-x^2) / (1+x^2), x, 0, 10, 3,'epsrel=5d-8);
  (%o1)	[0.6716467108233667,1.2146370412561506*10^-10,93,0]
  (%i6)	%e*\sqrt(%pi)/2*gamma_incomplete(1/2,1), numer;
  (%o6)	0.6716467108233667
\end{verbatim}


\paragraph{Different Solution.} ChatGPT (I know ;-) ) provides a different way. It parametrizes as

\bee
I(a) = \int_0^\infty \frac{e^{-a x^2}}{1+x^2} dx
\eee

Differentiation wrt $a$ yields

\bee
\frac{d}{da} I(a) = \int_0^\infty \frac{\partial}{\partial a} \frac{e^{-a x^2}}{1+x^2} dx = - \int_0^\infty \frac{x^2 e^{-a x^2}}{1+x^2} dx
\eee

We can expand the fraction and obtain

\bee
\frac{d}{da} I(a) = - \int_0^\infty e^{a x^2} dx + \int_0^\infty \frac{e^{-a x^2}}{1+x^2} dx
\eee

The first integral is "known" and the second integral is $I(a)$, therefore

\bee
\frac{d}{da} I(a) = - \frac{1}{2} \sqrt{\frac{\pi}{a}} + I(a)
\eee

We can solve the ODE (it's a bit tricky, but nothing special) and arrive at

\bee
I(a) = \frac{\pi}{2} e^a \erfc(\sqrt{a})
\eee

with the special case

\bee
I = I(1) = \frac{\pi}{2} e \erfc(1)
\eee


\subsection{Angle of Intersection}

We have two $2-$d cuves, $y = f_1(x)$ and $y = f_2(x)$. Assume they intersect at a point $(x_0, y_0)$ and we want to obtain the intersection angle.

We calculate the first derivative of each curve; $m_1 = f'_1(x_0)$ and $m_2 =f'_2(x_0)$ and calculate the intersection angle $\theta$ according to

\bee
\tan \theta = \tan (\alpha_2 - \alpha_1 )
\eee

where $\tan \alpha_i = m_i$. We can rewrite the difference inside the $\tan$ and arrive at

\bee
\tan \theta = \frac{\tan \alpha_2 - \tan \alpha_1}{1 + \tan \alpha_1 \tan \alpha_2}
\eee

Inserting back the derivatives, we arrive at

\bee
\theta = \arctan \frac{m_2 - m_1}{1 + m_1 m_2}
\eee

There are two special cases: (i) The derivatives are the same $m_1 = m_2$ and we have $\theta = \arctan 0 = 0 \degree$, (ii) the tangents meet at right angle; ie $m_1 m_2 = -1$ and we obtain $\arctan \infty = 90 \degree$.

\paragraph{Simple Example.} Let's consider $f_1(x) = 2x+1, f_2(x) = 10-x$. The two curves intersect at

\bee
2x+1 = 10-x \rightarrow 3x = 9 \rightarrow x_0 = 3, y_0 = 7
\eee

The derivates are constant, $m_1 = 2, m_2 = -1$ and therefore the intersection angle becomes

\bee
\theta = \arctan \frac{2 + 1}{1 - 2} = \arctan (-3) \approx -71.6 \degree
\eee

I made a construction in desmos and plotted the result (together with the angle measurement) below. This confirms the angle calculation made above.

\begin{figure}[H]
\centering
\includegraphics[scale=0.5]{images/2026-01-13-complex_analysis_02.png}
\end{figure}

\paragraph{Second Example.} Let's next consider two curves which are not lines: $f_1(x) = x^2, f_2(x) = 8 - x^2$. We start with the interception point

\bee
x^2 = 8-x^2 \rightarrow x_0 = \pm 2, y_0 = 4
\eee

The derivatives of the two functions are $f'_1(x) = 2x, f'_2(x) = -2x$ and their values at the right interception point, $x=2$, are

\bee
m_1 = 4, m_2 = -4
\eee

From this we can calculate the angle at the intersection as

\bee
\theta = \arctan \frac{-4 - 4}{1 - 16} = \frac{-8}{-15} \approx 28.1 \degree
\eee

The construction in Desmos is shown below and again confirms the calculated result.

\begin{figure}[H]
\centering
\includegraphics[scale=0.5]{images/2026-01-13-complex_analysis_03.png}
\end{figure}

\subsection{Integration Fun}

I stumbled across the following integral somewhere,

\begin{equation*}
  I = \int f(x) dx = \int x \cdot e^{-x^2} dx
\end{equation*}

We substitute $u = x^2 \rightarrow dx = du / 2x$ and therefore

\begin{equation*}
  I = \int x \cdot e^{-u} \frac{du}{2x} = \frac{1}{2} \int x \cdot e^{-u} du = - \frac{1}{2} e^{-x^2}
\end{equation*}

We can confirm this result with Maxima. The area under $f(x)$ for $x \geq 0$ is therefore given by

\begin{equation*}
  \int_0^\infty x \cdot e^{-x^2} dx = \left. - \frac{1}{2} e^{-x^2}\right|_0^\infty = \frac{1}{2} \qed
\end{equation*}


In order to get a feeling for the function, we observe that $f(0) = 0$ and differentiation yields

\begin{equation*}
  \frac{df(x)}{dx} = e^{-x^2} - 2x^2 e^{-x^2}
\end{equation*}

At $x = 0$, this is equal to $1$. Setting $\frac{df(x)}{dx} = 0$ we obtain 

\begin{equation*}
  e^{-x^2} - 2x^2 e^{-x^2} = e^{-x^2} (1 - 2x^2 ) = 0 \rightarrow x = \pm \frac{1}{\sqrt{2}}
\end{equation*}

The function value does not attain any nice expression at these points; nevertheless we are now able to have a rather clear picture how the function looks like (together with the fact that $\lim_{x \rightarrow \pm \infty} f(x) = 0$), which is also shown in the following Figure.

\begin{figure}[H]
\centering
\includegraphics[scale=0.6]{images/incubator_21_01.png}
\end{figure}

The more or less surprising extension is the integral

\begin{equation*}
  I = \int f(x) dx = \int x^3 \cdot e^{-x^2} dx
\end{equation*}

We substitute $u = x^2 \rightarrow dx = du / 2x$ and obtain

\begin{equation*}
  I = \int u x \cdot e^{-u} \frac{du}{2x} = \frac{1}{2} \int u \cdot e^{-u} du
\end{equation*}

We continue using integration by parts: $\int f g' = fg - \int f' g$ with $f=u, g'=e^{-u}$ from which we get $f'=1, g = - e^{-u}$ and therefore

\begin{equation*}
  \int u \cdot e^{-u} du = - u e^{-u} + \int e^{-u} du = - u e^{-u} - e^{-u} = - e^{-u}(u+1)
\end{equation*}

Inserting back into the originl integral, we get

\begin{equation*}
  I = \frac{1}{2} \int u \cdot e^{-u} du = - \frac{1}{2} e^{-u}(u+1) = - \frac{1}{2} e^{-x^2}(1 + x^2) \qed
\end{equation*}

Integrating over the right $x$-axis yields

\begin{equation*}
  \int_0^\infty x^3 \cdot e^{-x^2} dx = \frac{1}{2} e^{-0^2}(1 + 0^2) = \frac{1}{2} \qed
\end{equation*}

Differentiation yields

\begin{equation*}
  \frac{df(x)}{dx} = e^{-x^2} (3x^2 - 2 x^4)
\end{equation*}

which is zero at $x = 0$ - and different than with the function from before. Setting the derivative to zero yields

\begin{equation*}
  \frac{df(x)}{dx} = 0 \rightarrow x^2(3 - 2x^2) = 0 \rightarrow x = 0, x = \pm \sqrt{ \frac{3}{2}}
\end{equation*}

We could calculate the second derivative to find out that $x=0$ is not a minimum / maximum, or we could simply plot the function as below\dots

\begin{figure}[H]
\centering
\includegraphics[scale=0.6]{images/incubator_21_02.png}
\end{figure}
