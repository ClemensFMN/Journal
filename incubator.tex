...
The total length of travel $d$ is given according to

\bee
d = x + \sqrt{(H - x)^2 + W^2}
\eee

The shortest path $d_0$ is achieved when we use the  diagonal for the whole trip. This equals $x=0$ and we have
...
\bee
r = \frac{d_H}{d_0} = \frac{H + W}{\sqrt{H^2 + W^2}} = \frac{1 + W / H}{\sqrt{1 + (W/H)^2}} = \frac{1 + \alpha}{\sqrt{1 + \alpha^2}}
\eee

where we have set $\alpha = W/H$. \todo{maximum is at x=1, show that}
