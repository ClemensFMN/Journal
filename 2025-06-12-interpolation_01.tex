...
The following Figure shows the plot of the function $e^x$ and the interpolation; note that both intersect at the interpolation points, but the interpolated function differs elsewhere. This should not be surprising; we have added no further constraints (such as a minimization of the mean squared interpolation error) as we will do below.
...
L3:legendre_p(3,x) / sqrt(integrate(legendre_p(3,x)^2, x,-1,1));
f:exp(x);
...
The Figure shows the approximation together with $e^x$; the function and its approximation match very closely.
...
We can also investigate whether the RUnge phenomenon happens here as well. As before, we make a Legendre interpolation for $1/(1+x^2)$:

\begin{verbatim}
    N:10
    legendres:makelist(legendre_p(k,x)  / sqrt(integrate(legendre_p(k,x)^2, x,-1,1)), k, 0, N)$
    f:1/(1+x^2);
    coeffs:makelist(integrate(f*legendres[k], x, -1, 1), k, 1, N);
    approx:sum( coeffs[i]*legendres[i], i, 1, N)$
    plot2d([approx, f], [x,-1.5,1.5], [plot_format, gnuplot])$
\end{verbatim}
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.75]{images/2025-06-12-runge_legendre.png}
\end{figure}

Inside the interval $[-1,1]$, the approximation is very good; however outside it is pretty bad (but this was to be expected). In any case, we don't observed a Runge phenomenon.
