...
\subsection{Transformation by $g(x) = \sqrt{x}$}

Somehow as a converse example, we consider $g(x) = \sqrt{x} \rightarrow x = g^{-1}(y) = y^2$. The derivative of the inverse is
...
\bee
f_Y(y) = 2 |y| \left[ f_X( \sqrt{x}) + f_X(- \sqrt{x}) \right]
\eee

\paragraph{Uniform Distribution.} With this choice of $g$ it makes only sense to consider a uniform distribution between $[0,1]$ and $f_Y(y)$ becomes

\bee
f_Y(y) = \begin{cases} 2y \quad 0 \leq y \leq 1 \\ 0 \qquad \text{otherwise} \end{cases}
\eee
...
If we redo the exercise with a uniform distribution in $[-1,1]$, we obtain
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.7]{images/2025-10-09-plt_4.png}
\caption{PDF}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.7]{images/2025-10-09-plt_5.png}
\caption{PDF}
\end{figure}
