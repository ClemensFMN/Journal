...
\begin{itemize}
    \item Define a curve
    \item Define an integral along a curve
    \item Analytic function: Integral does not depend on curve
\end{itemize}
...
\bee
I = \int_\gamma f(z) dz = \int_{a}^b f(z) z(t) \frac{dz}{dt} dt
\eee



\subsection{Example with analytic function}

We start with a simple example of the complex analytic function $w = f(z) = z^2$. We want to integrate from $z = 0$ to $z= 1+j$. We integrate along different curves (as we will show later, the curves do not matter as the function is analytic).
...
\subsection{Example with non-analytic function}

We consider the function $w = u+jv = Re(z) = Re(x+jy) = x$. We will use the Cauchy-Rieman equations \eqref{2026-01-13:eq1}to show that the function is not analytic. We first calculate the partial derivatives,
